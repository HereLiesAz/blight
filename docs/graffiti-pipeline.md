...
| `scripts/lib/demolitions.py` | Joins BlightStatus Demolitions + Permit Apps Demolition rows; classifies pending/permitted/completed |
| `scripts/lib/permits.py` | Per-`geopin` permit history aggregation (count_365d + recent types) |
| `scripts/ingest_demolitions.py` | Per-property demolition status + city-wide `demolitions` worksheet tab |
| `scripts/ingest_permits.py` | Per-property permit history aggregates |
